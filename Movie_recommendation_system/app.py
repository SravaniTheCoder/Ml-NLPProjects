...
st.sidebar.markdown("<h2 style= color:#FF2056;font-family:Verdana;>Movie <br>Recommendation <br>System</h2>",unsafe_allow_html=True)
st.sidebar.image(os.path.join(BASE_DIR,"assets","clapperboard.png"),width=160)
st.sidebar.text("Never Run Out of Movies")
st.sidebar.markdown("### 🔗 Connect")
st.sidebar.markdown("""
 🌐 [GitHub](https://github.com/Taj1920/ML-NLP_Projects)
                    
 💼 [LinkedIn](https://linkedin.com/in/tajuddin-shaik-0a5804245)
""")
...
tab1,tab2,tab3,tab4 = st.tabs(tabs=["Popular","Recommend","Latest Movies","About"])
with tab1:
    #popular movies
...
This **Movie Recommendation System** is built to help users discover movies based on their interests using a **content-based recommendation** approach.

The app analyzes movie information such as **genres**, **overview**, **cast**, **director**, and **keywords** to understand the similarity between movies. Based on the movie selected by the user, it recommends other movies that are textually and contextually similar using **TF-IDF vectorization** and **cosine similarity**.

The core movie data is sourced from the **TMDB 5000 Movies** dataset, which provides rich metadata for building accurate recommendations. To keep the application up to date, the app also integrates the TMDB API to fetch:

🎥 Latest released movies

🔥 popular movies

🖼️ High-quality movie posters and additional details

🔧 **Key Features:**

        1. Content-based movie recommendations

        2. Latest  movies using TMDB API

        3. Movie posters and detailed overviews

        4. Interactive and user-friendly Streamlit interface

This project demonstrates practical implementation of Natural Language Processing (NLP) concepts and machine learning similarity techniques, combined with real-time API integration, to build a modern and scalable movie recommendation system. 
        
### 👨‍💻 Developer
**Shaik Tajuddin**  
Python Developer & Trainer  
📌 Specializing in Data Analysis, AI, Machine Learning & Streamlit  
                """)
